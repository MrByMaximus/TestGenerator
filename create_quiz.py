...
class QuestionCreatorMoodle():
    ...
    def addHtmlAnswerBlockMultichoice(self, choiceList):
        html = f'<div class="ablock"> <div class="answer">'
    ...
    def addHtmlAnswerBlockShortAnswer(self, answer):
        html = f'<div class="ablock"> <span class="answer"><input type="text" size="80" class="form-control d-inline" /> <p> Правильный ответ: {answer} </span> </div> </div> </div> </div> </body></html>'
        f=open(self.htmlFilename, 'a', encoding="utf-8")
        f.write(html)
        f.close()
